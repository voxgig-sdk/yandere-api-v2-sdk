...
class PostEntity
  ...
  def make
    opts = @_entopts.dup
    PostEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = YandereApiV2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Post, Hash] the current Post data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = YandereApiV2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Post fields)
  def match_get
  ...
  #
  # @param reqmatch [PostListMatch, Hash, nil] match filter (any subset of
  #   Post fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Post>, Array] the matching Post items; raises YandereApiV2Error on failure
