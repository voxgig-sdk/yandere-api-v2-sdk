...
module YandereApiV2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = YandereApiV2Response.new({})
    ...
    if fetch_err
      ...
      response = YandereApiV2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = YandereApiV2Response.new(fetched)
    else
