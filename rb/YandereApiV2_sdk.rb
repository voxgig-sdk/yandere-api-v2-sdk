...
require_relative 'YandereApiV2_types'


class YandereApiV2SDK
  ...
  def initialize(options = {})
    ...
    feature_opts = YandereApiV2Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = YandereApiV2Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, YandereApiV2Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    YandereApiV2Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = YandereApiV2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = YandereApiV2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = YandereApiV2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => YandereApiV2Error.new(
        "#{op}_allow",
        "YandereApiV2SDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = YandereApiV2Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = YandereApiV2Error.new(
        "graphql_error", "YandereApiV2SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = YandereApiV2SDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
