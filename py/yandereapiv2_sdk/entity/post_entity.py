...
from yandereapiv2_sdk.yandereapiv2_types import (
    Post,
    PostListMatch,
)
...
class PostEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PostEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Post]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Post().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
