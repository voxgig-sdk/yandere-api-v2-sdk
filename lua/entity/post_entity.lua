-- YandereApiV2 SDK Post entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PostEntity = {}
PostEntity.__index = PostEntity


function PostEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PostEntity)
  self._name = "post"
  self._client = client
...
function PostEntity:get_name()
  return self._name
end


function PostEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PostEntity.new(self._client, opts)
end
...
function PostEntity:mark_deleted()
  self._deleted = true
end


function PostEntity:deleted()
  return true == self._deleted
end


function PostEntity:data_set(args)
  if args ~= nil then
...
function PostEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PostEntity:match_set(args)
  if args ~= nil then
...
function PostEntity:stream(action, args, callopts)
...
function PostEntity:_run_op(ctx, post_done)
  local utility = self._utility
