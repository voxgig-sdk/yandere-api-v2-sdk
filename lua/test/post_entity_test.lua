...
describe("PostEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Post(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Post(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set YANDERE_API_V2_TEST_POST_ENTID JSON to run live")
      return
    end
...
function post_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("YANDERE_API_V2_TEST_POST_ENTID")
  ...
  if env["YANDERE_API_V2_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["YANDERE_API_V2_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
