...
//     (\`new YandereApiV2SDK(...)\` / \`YandereApiV2SDK.test(...)\` become
//     \`YandereApiV2SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'YandereApiV2SDK'
...
const TEST_SEED = {"entity":{"post":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new YandereApiV2SDK(...)\` and any
// \`YandereApiV2SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+YandereApiV2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/YandereApiV2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
