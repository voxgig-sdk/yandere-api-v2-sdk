...
// YandereApiV2 SDK retry feature
...
class YandereApiV2RetryFeature extends YandereApiV2BaseFeature
{
    ...
    public function init(YandereApiV2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (YandereApiV2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(YandereApiV2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(YandereApiV2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
