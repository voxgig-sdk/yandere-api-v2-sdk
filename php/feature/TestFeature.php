...
class YandereApiV2TestFeature extends YandereApiV2BaseFeature
{
    ...
    public function init(YandereApiV2Context $ctx, array $options): void
    {
        ...
        $test_fetcher = function (YandereApiV2Context $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = YandereApiV2Param::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (YandereApiV2Context $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(YandereApiV2Context $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = YandereApiV2Param::call($ctx, $k);
