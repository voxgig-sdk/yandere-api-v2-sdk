...
class PostEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = YandereApiV2SDK::test($seed, null);
        $seen = iterator_to_array($base->Post(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = YandereApiV2Config::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = YandereApiV2SDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Post(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set YANDERE_API_V2_TEST_POST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function post_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("YANDERE_API_V2_TEST_POST_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["YANDERE_API_V2_TEST_POST_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["YANDERE_API_V2_TEST_LIVE"] === "TRUE") {
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["YANDERE_API_V2_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
