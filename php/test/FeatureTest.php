...
class FtCtrl extends YandereApiV2Control
{
...
class FtClient
{
    ...
    private YandereApiV2Utility $utility;

    public function __construct(YandereApiV2Utility $utility, array $options)
    {
    ...
    public function get_utility(): YandereApiV2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (YandereApiV2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public YandereApiV2Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (YandereApiV2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = YandereApiV2Config::make_feature($name);
    ...
    private static function buildUrl(YandereApiV2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof YandereApiV2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new YandereApiV2Response($fetched);
        }
    ...
    private function fail(YandereApiV2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (YandereApiV2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
