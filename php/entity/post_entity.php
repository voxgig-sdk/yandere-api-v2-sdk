...
class PostEntity
{
    ...
    {
        if ($args) {
            $this->_data = YandereApiV2Helpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = YandereApiV2Helpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws YandereApiV2Error on failure (item-5 convention).
     */
