...
[MCP](https://modelcontextprotocol.io) server exposing the YandereApiV2 SDK as
two agent tools — `yandere-api-v2_list` and `yandere-api-v2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yandere-api-v2-mcp)
make build

# 2. Provide credentials via the environment
export YANDERE_API_V2_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user yandere-api-v2 \
  -- /absolute/path/to/yandere-api-v2-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./yandere-api-v2-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// yandere-api-v2_list: first page of records
{ "entity": "post" }
{ "entity": "post", "query": { } }

// yandere-api-v2_load: one record by id
{ "entity": "post", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yandere-api-v2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user yandere-api-v2 \
     -- "$PWD"/dist/*/yandere-api-v2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `yandere-api-v2_list` and `yandere-api-v2_load` tools now appear
   in new sessions. Ask the agent to *"list post using yandere-api-v2"*
   and it calls `yandere-api-v2_list` with `{"entity":"post"}`.
...
```sh
export YANDERE_API_V2_APIKEY=sk_live_xxx            # API key
export YANDERE_API_V2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `yandere-api-v2_list` tool
...
### Call the `yandere-api-v2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `yandere-api-v2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `yandere-api-v2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `YANDERE_API_V2_APIKEY` | API key sent with every request. |
| `YANDERE_API_V2_BASE` | Optional override of the API base URL. |
...
```sh
./yandere-api-v2-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"yandere-api-v2_load","arguments":{"entity":"post","query":{"id":1}}}}'
```
