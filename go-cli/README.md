...
boru-driven command-line client **and** interactive REPL for the YandereApiV2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yandere-api-v2-cli)
make build

# 2. See usage (words, entities, env vars)
./yandere-api-v2-cli --help

# 3. Provide credentials once, via the environment
export YANDERE_API_V2_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./yandere-api-v2-cli list post

# 5. Override the API base URL for a single call
YANDERE_API_V2_BASE=https://api.example.com ./yandere-api-v2-cli list post

# 6. No arguments -> interactive REPL
./yandere-api-v2-cli
yandere-api-v2> list post
yandere-api-v2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yandere-api-v2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/yandere-api-v2-cli list post
   ```
...
Run with no arguments to open a REPL (prompt `yandere-api-v2>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `post`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/yandere-api-v2-cli`. |
...
(`./yandere-api-v2-cli <expr>`) and interactively (the REPL), and expressions compose the
