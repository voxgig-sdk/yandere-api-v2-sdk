...
import (
	...
	omni "github.com/voxgig-sdk/yandere-api-v2-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.YandereApiV2Error); is {
				out := map[string]any{
					"name":    "YandereApiV2Error",
...
func omniCtx(arg any, client *sdk.YandereApiV2SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.YandereApiV2SDK); is {
			client = live
