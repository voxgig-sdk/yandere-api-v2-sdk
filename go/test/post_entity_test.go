...
func TestPostEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Post(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Post(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		postRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.post")))
...
func postBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YANDERE_API_V2_TEST_POST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YANDERE_API_V2_TEST_POST_ENTID"])
